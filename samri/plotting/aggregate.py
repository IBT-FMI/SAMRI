...
def roi_distributions(df,
	...
	lw = mpl.rcParams['lines.linewidth']
	g.map(sns.kdeplot, value_label, clip_on=False, gridsize=500, shade=True, alpha=1, lw=lw/4.*3, bw_method=bw)
	g.map(sns.kdeplot, value_label, clip_on=False, gridsize=500, color="w", lw=lw, bw_method=bw)
