def test_extract_volume(tmp_path):
	...
	out_file_name = f'{tmp_path}/test_extract_volume.nii.gz'
...
def test_flip_if_needed(tmp_path):
	import os
	import pandas as pd
...
def test_write_bids_physio_file(tmp_path):
	...
	write_bids_physio_file(scan_path,
		nii_name=f'{tmp_path}/sub-5691_ses-ofMpF_task-CogB_acq-EPI_cbv.nii.gz',
		)

def test_reset_background(tmp_path):
	...
	reset_background('/usr/share/samri_bidsdata/preprocessing/sub-4007/ses-ofM/func/sub-4007_ses-ofM_task-JogB_acq-EPIlowcov_run-1_cbv.nii.gz',
		...
		bg_value=1000,
		out_file=f'{tmp_path}/reset_background.nii.gz'
		)
	img = nib.load(f'{tmp_path}/reset_background.nii.gz')
	data = img.get_data()
