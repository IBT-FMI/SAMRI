...
def plot_design(
	...
	ax[0,0].plot(range(len(irf)), irf, irfcolor, alpha=0.8, label='gamma pdf')
	ax[0,0].yaxis.get_major_formatter().set_powerlimits((0, 1))
	ax[0,0].set_xlabel("time [s]")
	ax[0,0].set_title("IRF", color=irfcolor)

	ax[0,1].plot(x, design, stimcolor, alpha=0.8, label='gamma pdf')
	plt.setp(ax[0,1].xaxis.get_majorticklabels(), rotation=30)
	ax[0,1].set_title("Stimulaton", color=stimcolor)

	ax[0,2].plot(x, convoluted, regressorcolor, alpha=0.8, label='gamma pdf')
	plt.setp(ax[0,2].xaxis.get_majorticklabels(), rotation=30)
	ax[0,2].set_title(u"Stimulation × IRF", color=regressorcolor)

	ax[0,3].plot(x, filtered, regressorcolor, alpha=0.8, label='gamma pdf')
	plt.setp(ax[0,3].xaxis.get_majorticklabels(), rotation=30)
	...
	ax[1,1].plot(f_design, Pxx_den_design, stimcolor, alpha=0.5, label='gamma pdf')
	ax[1,1].set_xscale("log")

	f_irf, Pxx_den_irf = signal.periodogram(irf, 1, "barthann")
	ax[1,0].plot(f_irf, Pxx_den_irf, irfcolor, alpha=0.5, label='gamma pdf')
	...
	ax[1,2].plot(f_convoluted, Pxx_den_convoluted, regressorcolor, alpha=0.5, label='gamma pdf')
	ax[1,2].set_xscale("log")

	f_filtered, Pxx_den_filtered = signal.periodogram(filtered, 1, "barthann")
	ax[1,3].plot(f_filtered, Pxx_den_filtered, regressorcolor, alpha=0.5, label='gamma pdf')
